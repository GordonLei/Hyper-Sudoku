...
class SudokuPuzzle():
    ...
    def backwardCheck(self):
        ...
        #Check for a number(1-9) that will not cause constraint conflicts
        for i in range(1,10):
            #check if the value we are checking is in the domain
            if i in self.coordinateDomains[coord[0]+coord[1]]:
                ...
                for row in self.coordinateValue:
                    line = " ".join(str(coord) for coord in row)
                #forward check to make sure there are no domain repeats
                ...
                if self.backwardCheck():
                    return True
                #reset values to what it was before
                self.coordinateValue[int(coord[0])][int(coord[1])] = 0
                self.coordinateDomains = temp
        return False
    ...
    def chooseNextCoordinate(self):
        ...
        #get spaces that do not currently have a value
        ...
        #Check the number of unassigned neighbor
            ...
            if (self.checkInOverlappingBlock(coord_x, coord_y)):
                ...
                for i in range(startx, endx, 1):
                    for j in range(starty, endy, 1):
                        if (i,j) not in checked_coord_ls and self.coordinateValue[i][j] == 0:
                                counter+=1
            #append the coordinate and the number of unassigned neighbors
            tempArrayUnassignedNeighborNum.append((coord,counter))
        #returns the coordinate with the most unassigned neighbors
            ...
            for j in range(9):
                if self.coordinateValue[i][j] != 0:      
                    if not (self.forwardCheck(i,j, self.coordinateValue[i][j])):
                        ...
                        quit()
        #run backward check after first checking puzzle is valid(not necessarily solvable)                 
        self.backwardCheck()
        self.print_solution()
